...
pub async fn get_metadata(state: web::Data<AppState>, path: web::Path<String>) -> impl Responder {
    ...
    match result {
        Ok(Some(rec)) => {
            // The DB size may be 0 if the STORE message didn't include one
            // (the spec says size is "generated by CCNs upon processing").
            // Fall back to the actual file on disk.
            let size = if rec.size > 0 {
                rec.size
            } else {
                state.file_store.size(&rec.hash).unwrap_or(0) as i64
            };
    ...
    match result {
        Ok(Some(msg)) => {
            ...
            let size = match file_result {
                Ok(Some(rec)) if rec.size > 0 => rec.size,
                _ => state.file_store.size(&file_hash).unwrap_or(0) as i64,
            };

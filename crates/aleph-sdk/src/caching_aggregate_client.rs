...
//! memoizes every aggregate read that flows through
//! [`AlephAggregateClient::get_aggregate`] (keyed by address and key), so each
...
//! [`get_all_aggregates`]) and the per-address aggregates that need bespoke
//! empty-response handling (websites, domains, port-forwarding) delegate to the
//! inner client unchanged; only single-key [`get_aggregate`] reads are cached.
//!
...
mod tests {
    ...
    /// times each key is fetched, so tests can assert the cache collapses
    /// repeat reads. The first fetch of `fail_first_key` returns an error to
    /// exercise the errors-are-not-cached path.
    ...
    impl CountingClient {
        fn new(fail_first_key: Option<&'static str>) -> Self {
            Self {
    ...
    async fn fetches_each_aggregate_once_across_repeated_calls() {
        let inner = CountingClient::new(None);
        let client = CachingAggregateClient::new(&inner);

        // Both the typed convenience methods and a raw get_aggregate for the
        // same key should share the one cached entry.
        for _ in 0..3 {
    ...
    #[tokio::test]
    async fn caches_per_key_independently() {
        let inner = CountingClient::new(None);
        let client = CachingAggregateClient::new(&inner);
    ...
    async fn does_not_cache_a_failed_fetch() {
        let inner = CountingClient::new(Some(SETTINGS_KEY));
        let client = CachingAggregateClient::new(&inner);

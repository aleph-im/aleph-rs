...
pub fn read_new_password() -> Result<Zeroizing<String>> {
    ...
    if !std::io::stdin().is_terminal() {
        bail!(
            "cannot prompt for a password without a terminal; \
             set the {PASSWORD_ENV_VAR} environment variable"
        );
    }
...
pub fn unlock_keystore(ks: &KeystoreV3, label: &str) -> Result<Zeroizing<[u8; 32]>> {
    ...
    if !std::io::stdin().is_terminal() {
        bail!(
            "account '{label}' is password-protected and no terminal is available; \
             set the {PASSWORD_ENV_VAR} environment variable"
        );
    }
    for attempt in 1..=MAX_ATTEMPTS {
        let p = Zeroizing::new(
            rpassword::prompt_password(format!("Password for account '{label}': "))

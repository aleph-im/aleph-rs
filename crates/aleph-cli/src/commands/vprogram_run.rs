...
use aleph_sdk::vprogram::bundle::PlatformArtifacts;
...
fn boot_spec(build: &LocalBuild, args: &VProgramRunArgs, accel: Accel) -> Result<LocalBootSpec> {
    // Every V-Program runtime manifest declares the platform pair, so the
    // vprogram fetcher always extracts it; only the instance flavor leaves
    // it out, and that flavor never reaches this command.
    let platform = build
        .artifacts
        .platform
        .as_ref()
...
fn disk_order(
    ...
    disks.push(platform.rootfs.clone());
    disks.push(platform.hash_tree.clone());
...
mod tests {
    ...
    fn disk_order_is_platform_then_workload_then_volumes_in_flag_order() {
        let platform = PlatformArtifacts {
            rootfs: PathBuf::from("/bundle/platform_rootfs"),
            hash_tree: PathBuf::from("/bundle/platform_hash_tree"),
        };
        ...
        assert_eq!(
            disk_order(&platform, &verity("workload"), &volumes),
            vec![

...
use crate::cli::{
    ImageRef, PlatformRequirement, StorageEngineCli, VProgramBuildArgs, VProgramCallArgs,
    VProgramCommand, VProgramCreateArgs, VProgramDeleteArgs, VProgramListArgs, VProgramShowArgs,
};
use crate::common::{
...
async fn upload_pair(
    ...
    let data_message = match published {
        ...
        None => upload_file(client, account, owner, &v.data, json, dry_run, engine).await?,
    };
    Ok(UploadedPair {
        data_message,
        tree_message: upload_file(client, account, owner, &v.hash_tree, json, dry_run, engine)
            .await?,
    })
}

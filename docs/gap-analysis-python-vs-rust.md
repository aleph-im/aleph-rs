...
| Get messages iterator (auto-paginate) | ✅ `get_messages_iterator()` | ❌ | Python auto-pages; Rust requires manual pagination |
...
| Get multiple aggregates | ✅ `get_aggregates()` (multi-key) | ❌ | Rust only fetches one key at a time |
...
| Posts iterator | ✅ `get_posts_iterator()` | ❌ | Auto-paging missing in Rust |
...
5. **Multi-key aggregate fetch** — `get_aggregates(address, keys=[...])`
